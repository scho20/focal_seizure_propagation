function [NBRE, NBRI] = Final_NeighborNetworkMotif_312(simt, dt, T, lenT)
        %% Description
        % This function serves to produce neighboring network currents
        % that can be added on to the currents transmitting across the ictal
        % neuronal network.
        
        % Inputs
        % simt: simulation time (ms)
        % dt: time step
        % T: time array
        % lenT: length of the time array
        
        % Outputs
        % NBRE: the current output array from the excitatory neighboring neurons
        % NBRI: the current output array from the inhibitory neighboring neurons
        ...
        fprintf('Neighboring Network: Starting simulation loop ...');
        tic;
    ...
    %% Outputs
    NBRE = sur_current_e(2,:);
    NBRI = sur_current_i(2,:);
end