...
close all;
%% Time Parameters
simt = 1000;         %simulation time (or 500)
...
% (4) Load surrounding currents
[NBRE, NBRI] = Final_NeighborNetworkMotif_312(simt, dt, T, lenT);

fprintf('Temporary pause. Check results for the neighboring network. \n')
pause;
...
while (t<simt)
    ...
    for i = setdiff(1:Ne,idx_stim_e)
        if ~isempty(ind_e{i})
            if k >= ind_e{i}
                sur_current_e(i,k) = NBRE(1,k-ind_e{i}+1);
            end
        end
    end
    ...
    for i = setdiff(1:Ni,idx_stim_i-Ne)
        if ~isempty(ind_i{i})
            if k >= ind_i{i}
                sur_current_i(i,k) = NBRI(1,k-ind_i{i}+1);
            end
        end
    end
...
xlabel('Time (ms)')
ylabel('Synaptic Current (pA)')
title('Currents of Excitatory Neighboring Neuron')
...
xlabel('Time (ms)')
ylabel('Synaptic Current (pA)')
title('Currents of Inhibitory Neighboring Neurons')
...
fprintf('Temporary pause to check results. \n'); pause;
%% Propagation Delays
